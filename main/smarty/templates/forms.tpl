	<div id="alert_dialog" title="Предупреждение" style="display: none;">
		<p>&nbsp;</p>
	</div> <!-- #alert_dialog -->
...
{if $page eq "account"}	
	<div id="prompt_form_account" title="Создание пользователя" style="display: none;">
		<form id="create_user_form" name="create_user_form" action="index.php?page={$page}&amp;type={$type}" method="post">
			<p>Введите логин:<br />
...
{if $page eq "colors"}
	<div id="prompt_form_color" title="Добавление цветовой схемы" style="display: none;">
		<form id="create_form_color" name="create_form_color" action="index.php?page=colors&amp;action=create" method="post">
			...
			<input type="hidden" name="base_scheme_id" id="base_scheme_id" value="{$scheme.id}" />
	...
	<div id="confirm_form_color" title="Удаление цветовой схемы" style="display: none;">
		<form id="delete_form_color" name="delete_form_color" action="index.php?page=colors&amp;action=delete" method="post">
			...
			{if $schemes}
				...
				<tr>
					<td><input type="checkbox" id="scheme-{$schemes[i].id}" name="scheme-{$schemes[i].id}" onclick="if (this.checked) this.value='{$schemes[i].title}'; else this.value='';" value="" /><label for="scheme-{$schemes[i].id}">{$schemes[i].title}</label></td>
				</tr>
				{/section}
			{else}
				<tr>
...
{if $page eq "pages"}
	<div id="prompt_form_page" title="Создание страницы" style="display: none;">
		<form id="create_page_form" name="create_page_form" action="index.php?page={$page}&amp;type={$type}" method="post">
...
{section name=i loop=$pages}
	<div id="prompt_page{$pages[i].id}_double" title="Дублирование страницы" style="display: none;">
		<form id="double_page{$pages[i].id}_form" name="double_page{$pages[i].id}_form" action="index.php?page=pages&amp;type=all&amp;action=double_page&amp;id={$pages[i].id}" method="post">
			<p>Введите название страницы:<br />
			<input type="text" class="input_text tooltip margin_top8" id="double_page{$pages[i].id}_name" name="double_page{$pages[i].id}_name" title="Короткое название страницы для отображения в списке страниц" /></p>
			<p>Введите имя файла страницы:<br />
			<input type="text" class="input_text tooltip margin_top8" id="double_page{$pages[i].id}_file" name="double_page{$pages[i].id}_file" title="Имя файла страницы латинскими буквами без расширения .html или .php" /></p>
			<p>Выберите нужные блоки:</p>
			<table class="page_double">
			{if $pages[i].blocks}
				{section name=j loop=$pages[i].blocks}
				<tr>
					<td><input type="checkbox" id="{$pages[i].blocks[j].form_name}" name="{$pages[i].blocks[j].form_name}" checked="checked" /><label for="{$pages[i].blocks[j].form_name}">{$pages[i].blocks[j].form_title}</label></td>
					<td><input type="radio" id="copy-{$pages[i].blocks[j].form_id}" name="type-{$pages[i].blocks[j].form_id}" checked="checked" value="copy" /><label for="copy-{$pages[i].blocks[j].form_id}">копия</label>{if $pages[i].blocks[j].mirror eq 0}&nbsp;&nbsp;<input type="radio" id="link-{$pages[i].blocks[j].id}" name="type-{$pages[i].blocks[j].id}" value="link" /><label for="link-{$pages[i].blocks[j].id}">ссылка</label>{/if}</td>
				</tr>
				{/section}
			{else}
				<tr>
	...
	</div> <!-- #prompt_page{$pages[i].id}_double -->
...
{/if}
{if ($page eq "pages" or $page eq "blocks") and ($type ne "all") and ($type ne "prototypes")}
	<div id="prompt_select_form" title="Выбор формы" style="display: none;">
		<form id="{$page}_select_form" name="{$page}_select_form" action="index.php?page={$page}&amp;type={$type}" method="post">
			<p id="list_title">&nbsp;</p>
			<p id="list_content">&nbsp;</p>
		</form>
	...
	<form id="delete_variable_form" name="delete_variable_form" action="index.php?page={$page}&amp;type={$type}&amp;action=delete_variable" method="post">	
		<p>Выберите переменную(ые) для удаления:</p>
		<table>
		{if $variables}
			{section name=i loop=$variables}
			<tr>
				<td><input type="checkbox" id="variable-{$variables[i].id}" name="variable-{$variables[i].id}" onclick="if (this.checked) this.value='{$variables[i].name}'; else this.value='';" value="" /><label for="variable-{$variables[i].id}">${$variables[i].name}</label></td>
			</tr>
			{/section}
		{else}
			<tr>
...
{if $page eq "blocks"}
	...
	<div id="prompt_form_block" title="" style="display: none;">
		<div id="select_part" style="display: none;">
			<p>
				<input type="radio" name="block_type" id="block_type_empty" value="block_empty" checked="checked" {literal}onchange="if (this.checked) {$('#prototype_desc').css('display','none'); $('#prototype_select').css('display','none'); $('#realblock_select').css('display','none'); $('#names_part').css('display','block');}"{/literal} /><label for="block_type_empty">Пустой блок</label><br />
				<input type="radio" name="block_type" id="block_type_mirror" value="block_mirror" {literal}onchange="if (this.checked) {$('#prototype_desc').css('display','none'); $('#prototype_select').css('display','none'); $('#realblock_select').css('display','block'); $('#names_part').css('display','none');}"{/literal} /><label for="block_type_mirror">Блок-ссылка</label><br />
				<input type="radio" name="block_type" id="block_type_full" value="block_base" {literal}onchange="if (this.checked) {$('#prototype_desc').css('display','none'); $('#prototype_select').css('display','block'); $('#realblock_select').css('display','none'); $('#names_part').css('display','block');}"{/literal} /><label for="block_type_full">Блок на основе протототипа</label>
				{if $is_superadmin}
				<br /><input type="radio" name="block_type" id="block_type_prototype" value="block_prototype" {literal}onchange="if (this.checked) {$('#prototype_desc').css('display','block'); $('#prototype_select').css('display','none'); $('#realblock_select').css('display','none'); $('#names_part').css('display','block');}"{/literal} /><label for="block_type_prototype">Блок-прототип</label>
				{/if}
			</p>
			...
			<p id="prototype_select" style="display: none;">
				<select name="prototype_file" id="prototype_file" class="input_text margin_top8" onchange="$('#new_block_title').val($('#prototype_file :selected').text()); $('#new_block_file').val($('#prototype_file :selected').val());" style="height: 25px;"><option value="" selected="selected" disabled="disabled">Выберите прототип:</option>{section name=i loop=$prototypes}<option value="{$prototypes[i].file}">{$prototypes[i].title}</option>{/section}</select>
			</p>
			<p id="realblock_select" style="display: none;">
				<select name="realblock_id" id="realblock_id" class="input_text margin_top8" style="height: 25px;"><option value="" selected="selected" disabled="disabled">Выберите блок:</option>{section name=i loop=$realblocks}<option value="{$realblocks[i].id}">{$realblocks[i].title}</option>{/section}</select>
			</p>
		</div>
		<div id="names_part">
			<p>
...
{if $page eq "images"}
	...
	<div id="prompt_images_move" title="Выбор папки" style="display: none;">
		<p>Выберите папку для перемещения:<br />
			...
			{if $catalogs}
			{section name=i loop=$catalogs}
				<option value="{$catalogs[i].name}">{$catalogs[i].name}</option>
			{/section}
			{else}
...
{if $page eq "settings" and $type eq "scripts"}
	...
	<form id="delete_script_form" name="delete_script_form" action="index.php?page=settings&amp;type=scripts&amp;action=delete" method="post">
		...
		{if $settings}
			...
			<tr>
				<td><input type="checkbox" id="script-{$settings[i].id}" name="script-{$settings[i].id}" onclick="if (this.checked) this.value='{$settings[i].title}'; else this.value='';" value="" /><label for="script-{$settings[i].id}">{$settings[i].title}</label></td>
			</tr>
			{/section}
		{else}
			<tr>
...
{/if}
{if ($page eq "templates" and $type ne "all") or ($page eq "blocks" and $type ne "all" and $type ne "active" and $type ne "inactive" and $type ne "prototypes")}
	...
	<form id="delete_style_form" name="delete_style_form" action="index.php?page={$page}&amp;type={$type}&amp;action=delete_style" method="post">	
		<p>Выберите css-переменную(ые) для удаления:</p>
		<table>
		{if $styles}
			{section name=i loop=$styles}
			<tr>
				<td><input type="checkbox" id="style-{$styles[i].id}" name="style-{$styles[i].id}" onclick="if (this.checked) this.value='{$styles[i].name}'; else this.value='';" value="" /><label for="style-{$styles[i].id}">${$styles[i].name}</label></td>
			</tr>
			{/section}
		{else}
			<tr>
	...
	<form name="jg_form" id="jg_form" action="index.php?page={$page}&amp;type={$type}" method="post">
				...
				<table>
					...
					<tr>
						...
						<td>
							...
							<a class="b" href="javascript:rgbch('bgcolor_link','b-')">–</a>]&nbsp;&nbsp;
							<input type="checkbox" id="bgcolor_link_no" name="bgcolor_link_no" {literal}onclick="if (this.checked) { apv('aHighlight','transparent'); apcl('links','backgroundColor','transparent');  } else { apv('aHighlight','#'+document.jg_form.bgcolor_link.value); apcl('links','backgroundColor','#'+document.jg_form.bgcolor_link.value); }"{/literal} checked="checked" /><label for="bgcolor_link_no">Нет</label>
						</td>
					</tr>
				</table>
				</fieldset>
				...
				<table>
					...
					<tr>
						...
						<td>
							...
							{if $list_images}{section name=i loop=$list_images}
							<option value="{$list_images[i].src}" class="option_image" style="background-image: url({$list_images[i].src});">{$list_images[i].file}</option>
							{/section}{/if}
							</select>&nbsp;&nbsp;
